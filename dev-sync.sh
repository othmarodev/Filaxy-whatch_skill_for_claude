#!/usr/bin/env bash
#
# dev-sync.sh — copy this working tree into the installed /filaxy-watch plugin cache so
...
# setting FILAXY_WATCH_INSTALL_PATH. Pass --dry-run to preview without writing.
#
# Usage:
...
PLUGIN_KEY="filaxy-watch@filaxy-watch"
...
if [[ -z "$DEST" ]]; then
  DEST="${FILAXY_WATCH_INSTALL_PATH:-}"
fi
if [[ -z "$DEST" ]]; then
