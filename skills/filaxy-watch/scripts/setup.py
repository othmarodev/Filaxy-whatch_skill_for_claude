#!/usr/bin/env python3
"""Setup / preflight for /filaxy-watch.
...
- Silent on success: --check exits 0 with no output when everything's ready so
  that /filaxy-watch doesn't spam "setup is complete" on every turn.
- Idempotent: re-running the installer is safe — it never clobbers existing
  keys and only appends missing ones.
- SETUP_COMPLETE=true in ~/.config/filaxy-watch/.env tells us the user has been
...
ENV_TEMPLATE = """# /filaxy-watch API configuration
#
# Whisper transcription fallback — used only when yt-dlp cannot get captions
# (or when you point /filaxy-watch at a local file with no subtitles).
#
...
#
# Leave both blank to disable Whisper — /filaxy-watch will still work, but videos
# without native captions will come back frames-only.

GROQ_API_KEY=
OPENAI_API_KEY=

# Default watch behavior (the /filaxy-watch first-run wizard sets this for you).
...
def _scaffold_env() -> bool:
    """Create ~/.config/filaxy-watch/.env with placeholders if missing."""
...
def _status() -> dict:
    ...
    `can_proceed` is the operational gate: /filaxy-watch can run as long as the
...
def cmd_check() -> int:
    ...
    Exit 0 with no output when /filaxy-watch can run. A keyless user who already
    ...
    On a state that blocks /filaxy-watch, print one actionable line to stderr:
      2 → binaries missing
...
def cmd_install() -> int:
    ...
    if has_key:
        ...
        if installed_deps:
            print("[setup] installed dependencies; /filaxy-watch is fully set up.")
        return 0
    ...
    print("")
    print("  Without a key, /filaxy-watch still works but videos without captions come back frames-only.")
    return 3
